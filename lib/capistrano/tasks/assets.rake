    ...
    task :backup_manifest do
      on roles(fetch(:assets_roles)) do
        ...
        within release_path.join(rails_dir) do
          execute :cp,
            release_path.join(rails_dir, 'public', fetch(:assets_prefix), 'manifest*'),
    ...
    task :restore_manifest do
      on roles(fetch(:assets_roles)) do
        ...
        within release_path.join(rails_dir) do
          ...
          target = capture(:ls, release_path.join(rails_dir, 'public', fetch(:assets_prefix),
                                                  'manifest*')).strip
...
namespace :load do
  task :defaults do
    ...
    set :linked_dirs, fetch(:linked_dirs, []).push("#{fetch(:rails_dir)}/public/#{fetch(:assets_prefix)}")
  end
end
